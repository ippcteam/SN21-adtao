...
class TestIgnoreAlreadyScored:
    ...
    # ------------------------------------------------------------------
    # SN21_OVERRIDE_WEIGHT_UID — operator escape hatch
    # ------------------------------------------------------------------
    ...
    def test_override_replaces_weights_with_single_uid(self, setup, monkeypatch):
        ...
        The explicit 1.0 became required when burn was wired to the operator's dated
        ...
        assert out.ok
        # The commit must have received the override vector, NOT the
        # computed per-miner vector (which would have 5 entries from
        # the test fixture's miner_inputs).
        assert captured["uids"] == [135]
        assert captured["weights"] == [1.0]

    def test_override_invalid_value_does_not_crash(self, setup, monkeypatch):
        """When SN21_OVERRIDE_WEIGHT_UID is set to an invalid value (non-int
        or out of u16 range), the run completes without crashing — the
        override is silently ignored and the normal scoring path runs.
        We only assert that the run did not raise; the final commit-vector
        shape depends on the fixture's downstream scoreability checks and
        is covered by happy-path tests."""
        monkeypatch.setenv("SN21_OVERRIDE_WEIGHT_UID", "not_a_number")
        ...
        with (
            ...
            # Must not raise — invalid override is silently ignored, default
            # scoring path runs, run completes cleanly.
            run_epoch_scoring(
                subtensor=object(),
                validator_wallet=_StubWallet(),
                netuid=21,
                epoch_id="EPOCH-BAD-OVERRIDE",
                epoch_idx=42,
                validator_hotkey=setup["val_pk"],
                validator_signing_key=setup["val_sk"],
                miner_inputs=setup["inputs"],
